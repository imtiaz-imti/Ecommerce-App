...
return res.status(statusCode).cookie('token',token,options).setHeader('Access-Control-Allow-Origin', '*').setHeader('Access-Control-Allow-Methods: GET, POST, PATCH, PUT, DELETE, OPTIONS').setHeader('Access-Control-Allow-Headers: Origin, Content-Type, X-Auth-Token').json({success:true,message:'user logged in successfully',token,userNew:userNew._id})
}
