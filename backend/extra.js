...
const sendToken = (userNew,statusCode,res)=>{
  const token = userNew.getJWTToken() || 'hello'
  return res.status(200).setHeader('Access-Control-Allow-Origin', 'https://ecommerce-sb7c.onrender.com').json({success:true,message:'user logged in successfully',token,userNew:userNew._id})
  const options = {
     expires: new Date(Date.now() + process.env.COOKIE_EXPIRES * 24 * 60 * 60 * 1000),
     httpOnly:true
  }
  return res.status(statusCode).setHeader('Access-Control-Allow-Origin', 'https://ecommerce-sb7c.onrender.com').cookie('token',token,options).json({success:true,message:'user logged in successfully',token,userNew:userNew._id})
}
