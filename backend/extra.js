...
return res.setHeader('Access-Control-Allow-Origin', 'https://ecommerce-sb7c.onrender.com').setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS').status(statusCode).cookie('token',token,options).json({success:true,message:'user logged in successfully',token,userNew:userNew._id})
}
