...
return res.setHeader('Access-control-allow-origin','*').status(statusCode).cookie('token',token,options).json({success:true,message:'user logged in successfully',token,userNew:userNew._id})
}
